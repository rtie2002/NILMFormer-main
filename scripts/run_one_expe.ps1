            ...
            foreach ($win in $WindowSizes) {
                foreach ($model in $Models) {
                    foreach ($seed in $SEEDS) {
                        ...
                        Write-Host "Running: python scripts\run_one_expe.py --dataset $dataset --sampling_rate 1min --appliance $appliance --window_size $win --name_model $model --seed $seed" -ForegroundColor Yellow
                        Write-Host "========================================`n" -ForegroundColor Cyan
