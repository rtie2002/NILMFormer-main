...
class NILMDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # CRITICAL FIX: Support both pre-loaded time features AND on-the-fly generation
        # Decision logic:
        # - If self.st_date is None → Use pre-loaded time features (run_one_direct.py)
        # - If self.st_date is not None → Generate from st_date (run_one_expe.py)
        if self.n_var is not None:
            if self.st_date is None:
                ...
                if self.samples.shape[2] >= 10:
                    ...
                    if np.any(preloaded_time != 0):
                        exo = preloaded_time  # Use YOUR pre-loaded data
                    else:
                        # Fallback: channels are empty, but we have no st_date to generate
                        raise ValueError("st_date is None but no pre-loaded time features found in channels 2-9")
                else:
                    raise ValueError("st_date is None but data array doesn't have 10 channels for time features")
            else:
                # Case 2: run_one_expe.py - Generate from st_date
                exo = self._create_exogene(idx)  # Generate on-the-fly
